...
<div class="page-container">
    ...
    <form action="<?= \yii\helpers\Url::toRoute('employee/index') ?>" method="post">
        <input name="_csrf" type="hidden" id="_csrf" value="<?= Yii::$app->request->csrfToken ?>">
        <div class="text-c"> 日期范围：
            <input type="text" onfocus="WdatePicker({ maxDate:'#F{$dp.$D(\'datemax\')||\'%y-%M-%d\'}' })" id="datemin" name="startdate" value="<?= $where['startdate'] ?>" class="input-text Wdate" style="width:120px;">
            -
    ...
    <div class="mt-20">
        <table class="table table-border table-bordered table-hover table-bg">
            ...
            <tr class="text-c">
                <td><input type="checkbox" value="<?= $employee->id ?>" name="id"></td>
                <td><?= $employee->id ?></td>
                <td><u style="cursor:pointer" class="text-primary" onclick="member_show('<?= $employee->username ?>','member-show.html','10001','360','400')"><?= $employee->username ?></u></td>
                <td><?= $employee->gender ?></td>
                <td><?= $employee->tel ?></td>
                <td><?= $employee->email ?></td>
                <td><?= date('Y-m-d', $employee->enroll_date) ?></td>
